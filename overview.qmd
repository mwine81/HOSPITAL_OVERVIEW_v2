...
state_data = pl.read_parquet(Path('data\state_check.parquet')).rename(rename_dict)
hospital_data = pl.scan_parquet(Path('data/hospital.parquet'))
ndc_data = pl.scan_parquet(Path('data/ndc_names.parquet'))
hcpcs_desc = pl.scan_parquet(Path('data/hcpcs_desc.parquet'))
```
...
``` {python}
show(pl.read_parquet(Path('data\hospital_check.parquet')))
```
